---
title: OcrPageRecognizeEventsArgs
second_title: Aspose.OCR 适用于 .NET API 参考
description: 用于 OcrProgress 事件识别参数的类。
type: docs
weight: 530
url: /zh/net/aspose.ocr.models.events/ocrpagerecognizeeventsargs/
---
## OcrPageRecognizeEventsArgs class

用于 OcrProgress 事件识别参数的类。

```csharp
public class OcrPageRecognizeEventsArgs : EventArgs
```

## 属性

| 名称 | 描述 |
| --- | --- |
| [CurrentPage](../../aspose.ocr.models.events/ocrpagerecognizeeventsargs/currentpage) { get; set; } | 已开始识别的页面。 |
| [Duration](../../aspose.ocr.models.events/ocrpagerecognizeeventsargs/duration) { get; set; } | 从识别过程开始到当前图像被识别的时刻所经过的时间。 |
| [FileName](../../aspose.ocr.models.events/ocrpagerecognizeeventsargs/filename) { get; set; } | 处理后的文件名。 |
| [OperationName](../../aspose.ocr.models.events/ocrpagerecognizeeventsargs/operationname) { get; set; } | 正在进行的事务名称。 |

### 另请参阅

* namespace [Aspose.OCR.Models.Events](../../aspose.ocr.models.events)
* assembly [Aspose.OCR](../../)

<!-- 请勿编辑：由 xmldocmd 为 Aspose.OCR.dll 生成 -->
