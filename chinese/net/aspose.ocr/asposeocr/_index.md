...
| [DetectDocumentLayout](../../aspose.ocr/asposeocr/detectdocumentlayout)(OcrInput) | 分析图像并识别其中不同类型的内容区域。支持来自文件、流和像素数组的 PNG、JPEG、BMP、TIFF、JFIF 和 GIF 图像。可以批量处理文件夹和压缩包。 |
...
| [ImageTextDiff](../../aspose.ocr/asposeocr/imagetextdiff)(string, string, RecognitionSettings, bool, bool) | 比较两幅图像上的文本并返回表示相似度的数值（0 到 1）。 |
...
| 名称 | 描述 |
| --- | --- |
| static [DebugMode](../../aspose.ocr/asposeocr/debugmode) | 启用调试模式。启用后，系统会保存中间图像处理结果，如预处理图像和绘制了文本行矩形的图像。 |
| static [DebugModeSaveDirectory](../../aspose.ocr/asposeocr/debugmodesavedirectory) | 调试结果将保存的目录。如果未设置，则默认使用当前工作目录。 |
