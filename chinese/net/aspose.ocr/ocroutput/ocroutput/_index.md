...
description: 使用空集合初始化 OcrOutputaspose.ocr/ocroutput 类的新实例。此构造函数调用基类构造函数以初始化空的 RecognitionResultaspose.ocr/recognitionresult 列表。
type: docs
weight: 10
...
使用空集合初始化 [`OcrOutput`](../../ocroutput) 类的新实例。此构造函数调用基类构造函数以初始化空的 [`RecognitionResult`](../../recognitionresult) 列表。
...
使用指定的初始容量初始化 [`OcrOutput`](../../ocroutput) 类的新实例。此构造函数创建一个预分配了预定义数量元素的 [`OcrOutput`](../../ocroutput)，在事先已知大小时可优化性能。
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 容量 | Int32 | 列表的初始容量。此数字表示列表在重新调整大小之前可以容纳的最大元素数量。 |
