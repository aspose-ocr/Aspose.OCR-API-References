...
| UNIVERSAL | `2` | 检测图像中的所有文本块，包括照片上稀疏和不规则的文字。是除表格和多列布局外大多数图像的通用选项。 |
| TABLE | `4` | 检测图像中的表格结构并提取各单元格的文字。推荐用于扫描的电子表格、报告以及其他基于表格的文档。 |
| CURVED_TEXT | `5` | 自动校正图像中弯曲的文字行，提升识别准确率并使更多文字得以恢复和提取。需要大量的处理能力和内存。 |
| FORMULA | `6` | 检测所有包含数学公式的块。 |

### 备注

在 [`RecognitionSettings`](../recognitionsettings) 中使用，以指定要识别的图像类型。
