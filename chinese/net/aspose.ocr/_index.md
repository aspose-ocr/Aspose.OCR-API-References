...
description: 该 Aspose.OCR 包含图像识别、倾斜校正、文本区域检测和拼写检查工具。
type: docs
weight: 10
url: /zh/net/aspose.ocr/
---
该 **Aspose.OCR** 包含图像识别、倾斜校正、文本区域检测和拼写检查工具。
...
| [ImageProcessing](./imageprocessing) | Aspose OCR 库的辅助类。允许对图像进行预处理和保存。 |
...
| [OcrOutput](./ocroutput) | 用于存储和管理 OCR 操作结果的容器类。 |
| [OcrRecognizeEventHandler](./ocrrecognizeeventhandler) | 用于跟踪多页图像识别进度的委托。 |
| [OCRTable](./ocrtable) | 表示包含已识别表格数据的单个表格。 |
...
| [RectangleOutput](./rectangleoutput) | 关于检测到的文本区域或行的数据。 |
| [Resources](./resources) | 管理可下载的资源，以增强 Aspose.OCR 的识别能力。 |
| [SkewOutput](./skewoutput) | 关于倾斜角度（以度为单位）和文件名的数据。 |
## 枚举
...
| [Country](./country) | 为了更好地识别护照，请指明护照所属国家。 |
| [DefectType](./defecttype) | 图像缺陷的类型。 |
| [DetectAreasMode](./detectareasmode) | 确定用于区域检测的神经网络类型。 |
| [ExecutionModeOnnx](./executionmodeonnx) | 控制是否希望在图中一次一次地执行算子或并发执行。 |
| [GraphOptimizationLevelOnnx](./graphoptimizationlevelonnx) | 使用 SessionOptions 的图优化级别。有关技术细节，请参阅 ONNX Runtime 文档。 |
| [InputType](./inputtype) | 用于处理/识别的图像/文档类型。 |
| [Language](./language) | 用于 OCR 的语言。ISO 939-3 代码。 |
| [LanguageDetectionLevel](./languagedetectionlevel) | 定义图像中文本识别的语言检测级别。 |
| [LoggingLevel](./logginglevel) | 定义要显示的日志类型。 |
| [PdfOptimizationMode](./pdfoptimizationmode) | 在降低图像质量的前提下显著减小 PDF 文件大小。 |
