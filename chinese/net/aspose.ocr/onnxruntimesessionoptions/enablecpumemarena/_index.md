...
description: 启用或禁用 ONNX Runtime 使用的 CPU 内存 arena 分配器。启用后，内存将被池化并重复使用以提升性能，但在多线程场景下可能导致内存消耗增加。禁用可在牺牲性能的情况下降低峰值内存使用。
type: docs
weight: 10
...
启用或禁用 ONNX Runtime 使用的 CPU 内存 arena 分配器。启用后，内存将被池化并复用以提升性能，但在多线程场景下可能导致内存消耗增加。禁用可在牺牲性能的情况下降低峰值内存使用。
