---
title: HuggingFaceRepoId
second_title: Aspose.OCR 适用于 .NET API 参考
description: HuggingFace 上模型的 ID，例如 openai/gpt2。如果指定，将从 HuggingFace 下载模型。默认 bartowski/Qwen2.5-3B-Instruct-GGUF。
type: docs
weight: 80
url: /zh/net/aspose.ocr.ai/asposeaimodelconfig/huggingfacerepoid/
---
## AsposeAIModelConfig.HuggingFaceRepoId property

HuggingFace 上模型的 ID（例如 "openai/gpt2"）。如果指定，将从 HuggingFace 下载模型。默认 bartowski/Qwen2.5-3B-Instruct-GGUF。

```csharp
public string? HuggingFaceRepoId { get; set; }
```

### 另请参阅

* class [AsposeAIModelConfig](../../asposeaimodelconfig)
* namespace [Aspose.OCR.AI](../../asposeaimodelconfig)
* assembly [Aspose.OCR](../../../)

<!-- 请勿编辑：由 xmldocmd 为 Aspose.OCR.dll 生成 -->
