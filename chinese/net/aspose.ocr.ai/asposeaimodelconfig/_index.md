...
| Name | 描述 |
| --- | --- |
| [AllowAutoDownload](../../aspose.ocr.ai/asposeaimodelconfig/allowautodownload) { get; set; } | 如果为 true，则在本地不存在模型时会自动下载模型。 |
| [ContextSize](../../aspose.ocr.ai/asposeaimodelconfig/contextsize) { get; set; } | 定义 LLM 在推理期间可用作上下文的最大 token 数量。如果为 null，将使用模型定义的默认上下文大小。较大的数值允许模型考虑更多文本，但可能需要更多内存。 |
