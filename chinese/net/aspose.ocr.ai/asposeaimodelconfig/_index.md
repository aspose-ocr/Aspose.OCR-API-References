...
| [ContextSize](../../aspose.ocr.ai/asposeaimodelconfig/contextsize) { get; set; } | 定义在推理期间 LLM 可用作上下文的最大 token 数量。如果为 null，将使用模型定义的默认上下文大小。更大的值允许模型考虑更多文本，但可能需要更多内存。 |
| [DirectoryModelPath](../../aspose.ocr.ai/asposeaimodelconfig/directorymodelpath) { get; set; } | 可选的路径，用于缓存已下载或已处理的模型。如果未设置，将使用默认的系统位置。 |
