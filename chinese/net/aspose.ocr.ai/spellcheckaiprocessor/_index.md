---
title: SpellCheckAIProcessor
second_title: Aspose.OCR 适用于 .NET API 参考
description: 使用 AsposeAI 对 OCR 识别结果进行拼写纠正的 AI 后处理器。
type: docs
weight: 600
url: /zh/net/aspose.ocr.ai/spellcheckaiprocessor/
---
## SpellCheckAIProcessor class

使用 AsposeAI 对 OCR 识别结果进行拼写纠正的 AI 后处理器。

```csharp
public class SpellCheckAIProcessor : IOcrAIPostProcessor
```

## 构造函数

| 名称 | 描述 |
| --- | --- |
| [SpellCheckAIProcessor](spellcheckaiprocessor)() | 默认构造函数。 |

## 方法

| 名称 | 描述 |
| --- | --- |
| [GetResult](../../aspose.ocr.ai/spellcheckaiprocessor/getresult)() | 获取应用拼写纠正后的 OCR 结果。 |
| [SaveTxt](../../aspose.ocr.ai/spellcheckaiprocessor/savetxt)(string) | 将纠正后的 OCR 结果保存为 TXT 文件。 |

### 另请参阅

* interface [IOcrAIPostProcessor](../iocraipostprocessor)
* namespace [Aspose.OCR.AI](../../aspose.ocr.ai)
* assembly [Aspose.OCR](../../)

<!-- 请勿编辑：由 xmldocmd 为 Aspose.OCR.dll 生成 -->
