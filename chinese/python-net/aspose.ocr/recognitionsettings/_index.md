...
| recognize_single_line | 设置单行图像识别。 <br/>            默认禁用 (false)。 <br/>            禁用所有与行分割相关的处理步骤。 <br/>            如果您的图像仅包含一行，请将此参数设为 true。禁用 RecognitionAreas 设置，因此所有区域设置将被忽略。 |
...
| upscale_small_font | 允许您使用专门针对小字体识别的额外算法。<br/>            对于字符尺寸较小的图像非常有用。 |
