...
| 名称 | 描述 |
| :- | :- |
| binarize() | 将图像转换为黑白图像。<br/>            二值图像是像素只有两种可能强度值的图像。 <br/>            它们通常显示为黑白。数值上，这两个值通常为黑色的 0 和白色的 255。<br/>            二值图像是通过对图像进行自动阈值处理生成的。 |
| binarize(area) | 将图像的一部分转换为黑白图像。<br/>            二值图像是像素只有两种可能强度值的图像。 <br/>            它们通常显示为黑白。数值上，这两个值通常为黑色的 0 和白色的 255。<br/>            二值图像是通过对图像进行自动阈值处理生成的。 |
...
| auto_denoising() | 启用额外的神经网络来改进图像——降低噪声。<br/>            适用于带有扫描伪影、失真、斑点、耀光、渐变、外来元素的图像。 |
| auto_denoising(area) | 启用额外的神经网络来改进图像部分——降低噪声。<br/>            适用于带有扫描伪影、失真、斑点、耀光、渐变、外来元素的图像。 |
| auto_skew() | 启用自动图像倾斜校正。 |
| auto_skew(area) | 启用自动图像部分倾斜校正。 |
| contrast_correction_filter() | 对比度校正滤波器。 |
| contrast_correction_filter(area) | 图像部分的对比度校正滤波器。 |
| to_grayscale() | 将图像转换为灰度图像。<br/>            灰度图像在图像中具有 256 级亮度（0 到 255）。 |
