...
| الاسم | الوصف |
| :- | :- |
| AsposeOcr() | ينشئ مثيلاً جديدًا من الفئة [AsposeOcr](/ocr/python-net/aspose.ocr/asposeocr/).<br/>            منشئ فارغ. |
## الخصائص
| الاسم | الوصف |
| :- | :- |
| set_debug_mode |  |
...
| الاسم | الوصف |
| :- | :- |
| recognize(images) | يتعرف على النص في الصور / المستندات.<br/>            يدعم GIF، PNG، JPEG، BMP، TIFF، JFIF، stream، directory، arrays، archives. |
| recognize(images, preset) |  |
| recognize(images, settings) | يتعرف على النص في الصور / المستندات.<br/>            يدعم GIF، PNG، JPEG، BMP، TIFF، JFIF، stream، directory، arrays، archives. |
...
| detect_rectangles(images) | يكتشف مناطق النص في الصور.<br/>            يدعم GIF و PNG و JPEG و BMP و TIFF و JFIF، التدفق، المجلد، المصفوفات، الأرشيفات. |
| detect_rectangles(images, areas_type, detect_areas) | يكتشف مناطق النص في الصور.<br/>            يدعم GIF و PNG و JPEG و BMP و TIFF و JFIF، التدفق، المجلد، المصفوفات، الأرشيفات. |
| recognize_characters(images) | يكتشف الرموز في الصور.<br/>            يدعم GIF و PNG و JPEG و BMP و TIFF و JFIF، التدفق، المجلد، المصفوفات، الأرشيفات. |
| recognize_characters(images, detect_areas_mode, language) | يكتشف الرموز في الصور.<br/>            يدعم GIF و PNG و JPEG و BMP و TIFF و JFIF، التدفق، المجلد، المصفوفات، الأرشيفات. |
...
| recognize_fast(images) | يتعرف على النص في الصور / المستندات.<br/>            يدعم GIF، PNG، JPEG، BMP، TIFF، JFIF، stream، directory، arrays، archives. |
| recognize_handwritten_text(images) | يتعرف على النص المكتوب بخط اليد في الصور. |
...
| calculate_skew(images) | يحسب زوايا الانحراف في الصور.<br/>            يدعم GIF، PNG، JPEG، BMP، TIFF، JFIF، التدفق، المجلد، المصفوفات، الأرشيفات. |
| detect_defects(images, defect_type) | يحدد تلقائيًا المناطق المشكلة في الصورة التي يمكن أن تؤثر بشكل كبير على دقة OCR.<br/>            يدعم صور PNG، JPEG، BMP، TIFF، JFIF، وGIF المقدمة كملف أو تدفق أو مصفوفة بكسل. يدعم التعرف الجماعي. |
...
| image_text_diff(full_path1, full_path2, settings, ignore_case, auto_skew) | قارن النصوص على الصورتين وأعد رقمًا يمثل مدى تشابههما (من 0 إلى 1). |
