---
title: AsposeAI
second_title: Aspose.OCR for .NET API Reference
description: Initializes a new instance of the AsposeAI class with default settings. Automatic model downloads are enabled.
type: docs
weight: 10
url: /net/aspose.ocr.ai/asposeai/asposeai/
---
## AsposeAI() {#constructor}

Initializes a new instance of the AsposeAI class with default settings. Automatic model downloads are enabled.

```csharp
public AsposeAI()
```

### See Also

* class [AsposeAI](../../asposeai)
* namespace [Aspose.OCR.AI](../../asposeai)
* assembly [Aspose.OCR](../../../)

---

## AsposeAI(ILogger) {#constructor_1}

Initializes a new instance of the AsposeAI class with a custom logger. Automatic model downloads are enabled.

```csharp
public AsposeAI(ILogger customLogger)
```

| Parameter | Type | Description |
| --- | --- | --- |
| customLogger | ILogger | The custom logger instance to use. Must inherit from the ILogger abstract class, and implement its abstract methods. ILogger |

### See Also

* class [AsposeAI](../../asposeai)
* namespace [Aspose.OCR.AI](../../asposeai)
* assembly [Aspose.OCR](../../../)

---

## AsposeAI(bool) {#constructor_2}

Initializes a new instance of the [`AsposeAI`](../../asposeai) class with default settings. Automatic model downloads are enabled.

```csharp
public AsposeAI(bool logging)
```

| Parameter | Type | Description |
| --- | --- | --- |
| logging | Boolean | Enables or disables internal console logging. Set to `true` to record detailed logs of AI operations, or `false` to run silently without logging. |

### See Also

* class [AsposeAI](../../asposeai)
* namespace [Aspose.OCR.AI](../../asposeai)
* assembly [Aspose.OCR](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.OCR.dll -->
